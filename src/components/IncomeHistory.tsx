...
export const IncomeHistory: React.FC<IncomeHistoryProps> = ({
  ...
  const handleApproveClick = (incomeId: string) => {
    ...
    // Also update the detail modal state so it reflects approval immediately
    if (selectedIncomeDetail && selectedIncomeDetail.id === incomeId) {
      ...
      {selectedIncomeDetail && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl relative space-y-4 border border-slate-200">
            ...
            {/* Approval Section — shown only for pending transactions to authorized approvers */}
            {selectedIncomeDetail.approvalStatus === 'प्रलंबित' && canApprove && (
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-amber-700 mt-0.5">
                      वरील जोडलेली पावती / स्क्रीनशॉट तपासून मग मंजुरी द्या.
                    </p>
                  </div>
                </div>
                <div className="flex gap-2">
                  <button
                    onClick={() => handleApproveClick(selectedIncomeDetail.id)}
