                      ...
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {item.attachmentUrl && (
                          <span
                            ...
                            className="px-1.5 py-0.5 bg-rose-100 text-rose-800 border border-rose-300 rounded text-[10px] font-bold flex items-center gap-1 cursor-pointer hover:bg-rose-200 shrink-0"
                            title="खर्च बिल/पावती जोडली आहे (Click to view attachment)"
                          >
      ...
      {selectedExpenseDetail && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl relative space-y-4 border border-slate-200">
            ...
            {/* Attached Expense Receipt / Screenshot Proof */}
            {selectedExpenseDetail.attachmentUrl && (
              <div className="space-y-1.5 pt-3 border-t border-slate-200">
                <span className="text-slate-700 font-bold text-xs flex items-center gap-1.5">
                  ...
                  <span>जोडलेले खर्च बिल / पावती / स्क्रीनशॉट (Expense Attachment Proof):</span>
                </span>
                {selectedExpenseDetail.attachmentUrl.startsWith('data:application/pdf') || selectedExpenseDetail.attachmentUrl.endsWith('.pdf') ? (
                  <a
                    href={selectedExpenseDetail.attachmentUrl}
                    target="_blank"
                    rel="noreferrer"
                    className="px-3 py-2 bg-rose-50 text-rose-900 border border-rose-300 rounded-xl text-xs font-bold flex items-center gap-2 hover:bg-rose-100 transition-colors w-fit"
                  >
                    <FileText className="w-4 h-4 text-rose-600" />
                    <span>📄 PDF बिल पाहा / डाउनलोड करा</span>
                  </a>
                ) : (
                  <div className="relative group rounded-xl overflow-hidden border border-slate-300 bg-slate-900 max-h-64 flex items-center justify-center">
                    <img
                      src={selectedExpenseDetail.attachmentUrl}
                      alt="खर्च बिल पुरावा"
                      className="max-h-60 object-contain cursor-pointer"
                      onClick={() => window.open(selectedExpenseDetail.attachmentUrl, '_blank')}
                    />
                    <a
                      href={selectedExpenseDetail.attachmentUrl}
                      target="_blank"
            ...
            >
              ...
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-2">
                <label className="block text-xs font-bold text-slate-700 flex items-center gap-1.5">
                  ...
                  <span>खर्च बिल / पावती पुरावा जोडा किंवा बदला (Attachment)</span>
                </label>
                <div className="flex items-center gap-2">
                  <label className="flex-1 flex items-center justify-center gap-2 p-2 bg-white border border-dashed border-slate-300 rounded-lg text-xs font-semibold text-slate-700 cursor-pointer hover:bg-rose-50">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        reader.onloadend = () => {
                          setEditingExpense({ ...editingExpense, attachmentUrl: reader.result as string });
                        };
                  ...
                  {editingExpense.attachmentUrl && (
                    <button
                      type="button"
                      onClick={() => setEditingExpense({ ...editingExpense, attachmentUrl: undefined })}
                ...
                {editingExpense.attachmentUrl && editingExpense.attachmentUrl.startsWith('data:image/') && (
                  <div className="w-16 h-16 rounded overflow-hidden border border-slate-300">
                    <img src={editingExpense.attachmentUrl} alt="Preview" className="w-full h-full object-cover" />
                  </div>
                )}
              </div>
